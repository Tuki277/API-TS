...
import { createTodo, deleteTodo, findAndUpdateTodo, findTodo, getAllTodo } from "../services/Todo.service"

export const getAllTodoController = async (req: Request, res: Response) => {
    const Todo = await getAllTodo({ valid: true });

    return res.status(200).json({ "Error": false, "data": Todo })
}

export const createTodoController = async (req: Request, res: Response) => {
    const post = await createTodo({ ...req.body })
...
export const deleteTodoController = async (req: Request, res: Response) => {
    ...
    const findTodoToDelete = await findTodo({ _id : id });
...
export const getTodoByIdController = async (req: Request, res: Response) => {
    ...
    if ( todoDetail == null) {
        ...
        return res.status(200).json({ "Error": false, "data": todoDetail })
    }
}